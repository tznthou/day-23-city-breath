...
| 措施 | 說明 |
|------|------|
| CSP / Rate Limiting / HTTPS | 由 Zeabur reverse proxy 統一處理 |
...
> Zeabur 會偵測 `main.py` 作為入口點。`main.py` 會自動判斷環境：本機開啟 debug mode，Zeabur 上關閉 debug 以節省記憶體。
> 安全性（CSP、Rate Limiting、HTTPS）由 Zeabur reverse proxy 處理，應用層不需額外 middleware。
